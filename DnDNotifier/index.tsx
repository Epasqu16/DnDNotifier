...
interface UserSound {
    userId: string;
    label: string;   // kullanıcının tanımlayıcı ismi (not)
    url: string;
    fileName: string;
}
...
const i18n = {
    en: {
        langLabel:        "Language",
        urlLabel:         "Default Sound URL",
        urlPlaceholder:   "https://files.catbox.moe/xxx.mp3",
        urlDesc:          "Enter a direct MP3/WAV/OGG link",
    ...
    tr: {
        langLabel:        "Dil",
        urlLabel:         "Varsayılan Ses URL'si",
        urlPlaceholder:   "https://files.catbox.moe/xxx.mp3",
                    ...
                    React.createElement(
                        ...
                        React.createElement(
                            "button",
                            {
                                ...
                                onClick: () => { settings.store.language = "en"; forceUpdate(); }
                            },
                            "🇬🇧  English"
                        ),
                        React.createElement(
                            "button",
                            {
                                ...
                                onClick: () => { settings.store.language = "tr"; forceUpdate(); }
                            },
                            "🇹🇷  Türkçe"
                        )
                    )
...
export default definePlugin({
    ...
    flux: {
        MESSAGE_CREATE(data: any) {
            ...
            // ── Kişiye özel ses var mı?
            const userSounds  = getUserSounds();
            const authorId    = message.author?.id ?? "";
            const perUser     = userSounds.find(u => u.userId === authorId);

            const defaultUrl  = settings.store.customSoundUrl?.trim();
            const soundToPlay = perUser?.url?.trim()
                ? perUser.url.trim()
